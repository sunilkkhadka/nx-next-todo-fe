'use client';
import {
  LaptopOutlined,
  MobileOutlined,
  TabletOutlined
} from '@ant-design/icons';
import { Card } from '@skeleton/shared';
import { Flex, Timeline, Typography } from 'antd';
import { UserProfileLayout } from '../user-profile-layout';
const data = [
  {
    user_id: '94a806a0-dba0-4739-a046-2dc93a0c1516',
    activity_type: 'like',
    timestamp: '03/02/2022',
    post_id: '94784a65-d6f3-4d0d-b6ad-f593eb679b50',
    post_content:
      'Integer ac leo. Pellentesque ultrices mattis odio. Donec vitae nisi.',
    comment_id: 1,
    comment_content:
      'Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Mauris viverra diam vitae quam. Suspendisse potenti. Nullam porttitor lacus at turpis. Donec posuere metus vitae ipsum. Aliquam non mauris. Morbi non lectus. Aliquam sit amet diam in magna bibendum imperdiet.',
    like_count: 350,
    location: 'Xinzhou',
    device_type: 'desktop'
  },
  {
    user_id: '3de3a689-a9a5-4be0-98f6-97293cf01cda',
    activity_type: 'comment',
    timestamp: '06/02/2022',
    post_id: 'e14d22e5-82bd-4317-b6a6-c9f339c204d4',
    post_content:
      'Vestibulum ac est lacinia nisi venenatis tristique. Fusce congue, diam id ornare imperdiet, sapien urna pretium nisl, ut volutpat sapien arcu sed augue. Aliquam erat volutpat.\n\nIn congue. Etiam justo. Etiam pretium iaculis justo.',
    comment_id: 2,
    comment_content:
      'Curabitur gravida nisi at nibh. In hac habitasse platea dictumst. Aliquam augue quam, sollicitudin vitae, consectetuer eget, rutrum at, lorem. Integer tincidunt ante vel ipsum. Praesent blandit lacinia erat.',
    like_count: 306,
    location: 'Regueiro',
    device_type: 'tablet'
  },
  {
    user_id: 'd59bf76a-407d-46c3-8218-681860487b7d',
    activity_type: 'login',
    timestamp: '07/26/2022',
    post_id: '5caa8dce-321b-469a-a460-560f77ae05ad',
    post_content:
      'Nullam sit amet turpis elementum ligula vehicula consequat. Morbi a ipsum. Integer a nibh.\n\nIn quis justo. Maecenas rhoncus aliquam lacus. Morbi quis tortor id nulla ultrices aliquet.',
    comment_id: 3,
    comment_content:
      'Nullam porttitor lacus at turpis. Donec posuere metus vitae ipsum. Aliquam non mauris. Morbi non lectus. Aliquam sit amet diam in magna bibendum imperdiet. Nullam orci pede, venenatis non, sodales sed, tincidunt eu, felis. Fusce posuere felis sed lacus.',
    like_count: 597,
    location: 'Rongcheng',
    device_type: 'mobile'
  },
  {
    user_id: 'da87cb1c-d70f-43e9-a0f5-e5a355494b20',
    activity_type: 'comment',
    timestamp: '04/02/2022',
    post_id: '20a8fa49-80a6-40fe-ae32-cc60f7c7a5ed',
    post_content:
      'Etiam vel augue. Vestibulum rutrum rutrum neque. Aenean auctor gravida sem.\n\nPraesent id massa id nisl venenatis lacinia. Aenean sit amet justo. Morbi ut odio.\n\nCras mi pede, malesuada in, imperdiet et, commodo vulputate, justo. In blandit ultrices enim. Lorem ipsum dolor sit amet, consectetuer adipiscing elit.',
    comment_id: 4,
    comment_content:
      'Morbi non quam nec dui luctus rutrum. Nulla tellus. In sagittis dui vel nisl. Duis ac nibh. Fusce lacus purus, aliquet at, feugiat non, pretium quis, lectus.',
    like_count: 791,
    location: 'Rostov-na-Donu',
    device_type: 'desktop'
  },
  {
    user_id: '805ad8b7-6622-415a-bdb3-455aea06d753',
    activity_type: 'logout',
    timestamp: '03/22/2022',
    post_id: 'ba8ad93a-6e38-41f5-b137-1b5f203d5aa4',
    post_content:
      'Nullam porttitor lacus at turpis. Donec posuere metus vitae ipsum. Aliquam non mauris.\n\nMorbi non lectus. Aliquam sit amet diam in magna bibendum imperdiet. Nullam orci pede, venenatis non, sodales sed, tincidunt eu, felis.\n\nFusce posuere felis sed lacus. Morbi sem mauris, laoreet ut, rhoncus aliquet, pulvinar sed, nisl. Nunc rhoncus dui vel sem.',
    comment_id: 5,
    comment_content:
      'Praesent lectus. Vestibulum quam sapien, varius ut, blandit non, interdum in, ante. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Duis faucibus accumsan odio. Curabitur convallis. Duis consequat dui nec nisi volutpat eleifend. Donec ut dolor.',
    like_count: 192,
    location: 'Gvardeysk',
    device_type: 'tablet'
  },
  {
    user_id: '172523b0-56c3-4e5d-8705-409474f687bb',
    activity_type: 'post',
    timestamp: '12/04/2022',
    post_id: 'e1855433-1434-480b-9ff2-25d539afc811',
    post_content:
      'Maecenas leo odio, condimentum id, luctus nec, molestie sed, justo. Pellentesque viverra pede ac diam. Cras pellentesque volutpat dui.\n\nMaecenas tristique, est et tempus semper, est quam pharetra magna, ac consequat metus sapien ut nunc. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Mauris viverra diam vitae quam. Suspendisse potenti.\n\nNullam porttitor lacus at turpis. Donec posuere metus vitae ipsum. Aliquam non mauris.',
    comment_id: 6,
    comment_content: 'In quis justo.',
    like_count: 506,
    location: 'Huanxi',
    device_type: 'mobile'
  },
  {
    user_id: '1200b501-79e8-4d01-a0dd-9916f33df627',
    activity_type: 'logout',
    timestamp: '02/22/2022',
    post_id: '2a43b0a5-b7e0-43f9-a75a-e93dff0b9a67',
    post_content:
      'Donec diam neque, vestibulum eget, vulputate ut, ultrices vel, augue. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Donec pharetra, magna vestibulum aliquet ultrices, erat tortor sollicitudin mi, sit amet lobortis sapien sapien non mi. Integer ac neque.\n\nDuis bibendum. Morbi non quam nec dui luctus rutrum. Nulla tellus.\n\nIn sagittis dui vel nisl. Duis ac nibh. Fusce lacus purus, aliquet at, feugiat non, pretium quis, lectus.',
    comment_id: 7,
    comment_content: 'Etiam faucibus cursus urna. Ut tellus.',
    like_count: 996,
    location: 'Drammen',
    device_type: 'desktop'
  },
  {
    user_id: '527ed1fe-d124-4e40-be42-28f557078d78',
    activity_type: 'logout',
    timestamp: '10/30/2022',
    post_id: '006746d3-2095-4fb2-b13c-d3464f460797',
    post_content:
      'Fusce consequat. Nulla nisl. Nunc nisl.\n\nDuis bibendum, felis sed interdum venenatis, turpis enim blandit mi, in porttitor pede justo eu massa. Donec dapibus. Duis at velit eu est congue elementum.',
    comment_id: 8,
    comment_content:
      'Suspendisse potenti. In eleifend quam a odio. In hac habitasse platea dictumst. Maecenas ut massa quis augue luctus tincidunt.',
    like_count: 262,
    location: 'Wudabao',
    device_type: 'desktop'
  },
  {
    user_id: 'c20ab445-3d09-46f3-9c3e-b1d2c69ab9ca',
    activity_type: 'login',
    timestamp: '10/04/2022',
    post_id: '99180644-3d14-40d7-86f5-346e0fe1fd3b',
    post_content:
      'Praesent id massa id nisl venenatis lacinia. Aenean sit amet justo. Morbi ut odio.',
    comment_id: 9,
    comment_content: 'Aliquam erat volutpat. In congue. Etiam justo.',
    like_count: 257,
    location: 'Bokor',
    device_type: 'desktop'
  },
  {
    user_id: 'e27c8eed-92f0-4408-a6da-95d25adb504f',
    activity_type: 'login',
    timestamp: '07/03/2022',
    post_id: 'fc7e7d7b-bace-4e81-a2fa-95b26f0a5cf0',
    post_content:
      'Aenean lectus. Pellentesque eget nunc. Donec quis orci eget orci vehicula condimentum.',
    comment_id: 10,
    comment_content:
      'Integer aliquet, massa id lobortis convallis, tortor risus dapibus augue, vel accumsan tellus nisi eu orci. Mauris lacinia sapien quis libero. Nullam sit amet turpis elementum ligula vehicula consequat. Morbi a ipsum. Integer a nibh. In quis justo. Maecenas rhoncus aliquam lacus. Morbi quis tortor id nulla ultrices aliquet. Maecenas leo odio, condimentum id, luctus nec, molestie sed, justo. Pellentesque viverra pede ac diam.',
    like_count: 760,
    location: 'San Juan Sacatepéquez',
    device_type: 'desktop'
  }
];

export const UserProfileActivityContainer = () => {
  return (
    <UserProfileLayout>
      <Card title="Latest activities">
        <Timeline
          mode="left"
          items={data?.map((_) => ({
            dot:
              _.device_type === 'desktop' ? (
                <LaptopOutlined />
              ) : _.device_type === 'tablet' ? (
                <TabletOutlined />
              ) : (
                <MobileOutlined />
              ),
            children: (
              <Flex gap="small" vertical>
                <Typography.Paragraph
                  ellipsis={{
                    rows: 2
                  }}
                  title={`${_.post_content}--${_.timestamp}`}
                  style={{ marginBottom: 0 }}
                >
                  {_.post_content}
                </Typography.Paragraph>
                <Typography.Text type="secondary">
                  {_.timestamp}
                </Typography.Text>
              </Flex>
            )
          }))}
        />
      </Card>
    </UserProfileLayout>
  );
};
